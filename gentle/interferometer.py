...
class PhaseCam():
    ...
    def _fromDataArrayToMaskedArray(self, width, height, data_array):
       # data = np.reshape(data_array, (width, height))
        data = _np.reshape(data_array, (height,width)) #mod20231002, rectangular frames were bad. now fixed
    ...
    def capture(self, numberOfFrames, folder_name=None):
        ...
        self._i4d.burstFramesToSpecificDirectory(_os.path.join(_folds.CAPTURE_FOLDER_NAME_4D_PC,
    ...
    def produce(self, folder_name):
        ...
        _sh.move(_os.path.join(_folds.PRODUCE_FOLDER_NAME_M4OTT_PC, folder_name),
